...
Es ist immer häufiger, Linux-Maschinen mit **schreibgeschütztem (ro) Dateisystemschutz** zu finden, insbesondere in Containern. Das liegt daran, dass es so einfach ist, einen Container mit ro Dateisystem zu starten, indem man **`readOnlyRootFilesystem: true`** im `securitycontext` festlegt:
...
Allerdings, selbst wenn das Dateisystem als ro gemountet ist, bleibt **`/dev/shm`** beschreibbar, sodass es falsch ist zu sagen, dass wir nichts auf die Festplatte schreiben können. Diese Ordner werden jedoch **mit no-exec-Schutz gemountet**, sodass du, wenn du hier eine Binärdatei herunterlädst, **sie nicht ausführen kannst**.
...
Wenn der kompromittierte Container jedoch beispielsweise eine Flask-Webanwendung ausführt, dann ist Python installiert, und daher können Sie eine **Python-Reverse-Shell** erhalten. Wenn es Node ausführt, können Sie eine Node-Reverse-Shell erhalten, und dasselbe gilt für die meisten **Skriptsprache**.
