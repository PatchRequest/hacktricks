...
{% hint style="success" %}
Cilj ove četvrte faze je vrlo jednostavan: **Primeni neke mehanizme pažnje**. Ovo će biti mnogo **ponovljenih slojeva** koji će **uhvatiti odnos reči u rečniku sa njenim susedima u trenutnoj rečenici koja se koristi za obuku LLM**.\
...
```
{% hint style="info" %}
Napomena da se umesto inicijalizacije matrica nasumičnim vrednostima, koristi `nn.Linear` da označi sve težine kao parametre za obuku.
{% endhint %}
...
```
3. **Primeni Softmax**: Izračunaj težine pažnje koristeći maskirane rezultate.
